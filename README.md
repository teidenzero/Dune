...
**1.2 The Banquet** follows 1.1 (launcher key `9`): the Lady Jessica's dinner for five notables of Arrakeen, and Paul answers them. Each guest secretly wants one of two things (or is the Harkonnen informant), and which of Paul's three replies is right depends on it. Watch each guest for his tell, or spend a help: look at your mother (how this guest will take each reply) or remember your father (what each reply means around the table), twice each an evening. Your parents react to what you say, and step in when you misstep. Keep the table's composure from running out, and at the end decide whether to name the informant. Reading a guest right on your own grows Paul's statecraft.

**Jumping around the training.** In either training, F10 lists every lesson; pick one and the training reloads straight into it.

**Briefings.** Every chapter you play, the trainings, the Council and the Banquet included, opens on a briefing screen: who is who, the situation, the objectives, what we know about the place, and the party with its equipment slots (the quartermaster is laid out but not open yet). Study it, then BEGIN (Enter); press `I` during the mission to read it again. During play, spoken lines are tactical only.

**1.3 The Harvester in the Open** follows the Banquet (launcher key `0`): a crawler is stranded in open sand, no carryall is coming, and a worm is already on its way (its arrival time is on the objectives). The worm homes on the loudest place on the sand. The running engine holds it at the crawler, and you can shut the engine down or restart it. Reach the eight crewmen: each of the squad leads two, and the injured one must be carried, slowly. Crouch and your men go low and quiet too. Nervous men bolt when the worm comes near. The ornithopters seat nine; the spice takes three of those seats if you load it (you can dump it on the rock). If Kynes trusts the House, `K` plants his thumper, louder than anything, to hold the worm where you want it. Hold LIFT OFF on the rock when you have everyone you can get. Give a unit a whole route with Shift + right-click (one click per stop); drag any waypoint to move it while the unit is in command range. A route given in range is carried out even beyond it.
