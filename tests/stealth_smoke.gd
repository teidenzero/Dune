...
func _crouch_and_pulses() -> void:
	...
	_check(squad.paul_selected, "Paul starts selected, so C reaches him")
	_key(KEY_C, true)
	await _frames(3)
	_key(KEY_C, false)
	...
	_check(hud.stealth_label.text.contains("CROUCHED"), "normal HUD reports crouch")
	_key(KEY_C, true)
	await _frames(2)
	_key(KEY_C, false)
	events.clear()
	await _frames(65)
	_check(not player.is_crouching and absf(player.current_speed - 180.0) < 1.0, "second C press restores walking")
	_check(_footsteps(80.0, 1) >= 2 and events.size() <= 3, "walking pulses use 80 px radius about twice per second")
	_key(KEY_C, true)
	await _frames(2)
	_key(KEY_C, false)
	_check(player.is_crouching, "C crouches again while walking")
	# Double right-click equivalent: the same destination, run.
	player.move_to(east, true)
	events.clear()
	await _frames(65)
	_check(not player.is_crouching and player.is_sprinting and absf(player.current_speed - 280.0) < 1.0, "run order exits crouch and reaches sprint speed (crouching=%s sprinting=%s speed=%.0f)" % [player.is_crouching, player.is_sprinting, player.current_speed])
	_check(_footsteps(160.0, 2) >= 3 and events.size() <= 4, "sprint pulses use 160 px and higher priority (%d of %d events)" % [_footsteps(160.0, 2), events.size()])
	# Standing Paul ordered to run (independent of the crouch hand-off above).
	player.set_crouching(false)
	player.move_to(Vector2(-900, -700), true)
	await _frames(25)
	events.clear()
	await _frames(65)
	_check(player.is_sprinting and absf(player.current_speed - 280.0) < 1.0, "standing run order reaches sprint speed")
	_check(_footsteps(160.0, 2) >= 3 and events.size() <= 4, "standing sprint pulses use 160 px and higher priority")
	player.stop()
	await _frames(20)
	events.clear()
	await _frames(40)
	...
	player.set_crouching(false)
	player.teleport_to(Vector2(1560, 950))
	player.move_to(Vector2(1700, 950))
	await _frames(30)
	events.clear()
	await _frames(45)
	_check(events.is_empty() and player.current_speed < 1.0, "ordering Paul into the boundary does not produce footsteps")
	player.stop()
	completed += 1
