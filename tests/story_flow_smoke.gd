...
func _run() -> void:
	...
	hud._on_launcher()
	await _frames(5)
	_check(game.flow.current().id == "banquet_card", "1.2's card")
	await _press(KEY_SPACE)
	await _frames(15)
	_check(_scene() == "res://scenes/campaign/banquet.tscn", "1.2 The Banquet (played through in banquet_smoke)")
	game.flow.advance(self)
	await _frames(5)
	_check(game.flow.current().id == "harvester_card", "1.3's card")
	await _press(KEY_SPACE)
	await _frames(20)
	_check(_scene() == "res://scenes/missions/act1/harvester_open.tscn", "1.3 The Harvester in the Open (played through in harvester_open_smoke)")
	game.flow.advance(self)
	await _frames(5)
